<template>
  <aside class="w-80 bg-[var(--color-surface)] border-l border-[var(--color-border)] flex flex-col z-10 shadow-sm flex-shrink-0">
    <div class="p-5 border-b border-[var(--color-border)]">
      <h2 class="text-sm font-bold text-[var(--color-text)] uppercase tracking-wider">AI Insights</h2>
    </div>
    <div class="flex-1 overflow-y-auto bg-[var(--color-background)] p-4 space-y-4">
      ...
      <div class="bg-[var(--color-surface)] rounded-xl shadow-sm border border-[var(--color-border)] p-4">
        <div class="flex items-center space-x-2 mb-3">
          ...
          <h3 class="text-xs font-bold text-[var(--color-text)] uppercase">Generated Summary</h3>
        </div>
        <p v-if="aiStore.lastGeneratedFlowJson" class="text-xs text-[var(--color-text-muted)] leading-relaxed">
          AI successfully generated a workflow based on your request.
          <br/><br/>
          <template v-if="aiStore.lastUsedTools && aiStore.lastUsedTools.length > 0">
            Added <span v-for="(tool, index) in aiStore.lastUsedTools" :key="index" class="font-mono text-red-500 bg-red-500/10 px-1 rounded mr-1 mb-1 inline-block">{{ tool }}</span>
          </template>
        </p>
      ...
      <!-- Tools Used -->
      <div class="bg-[var(--color-surface)] rounded-xl shadow-sm border border-[var(--color-border)] p-4" v-if="aiStore.lastGeneratedFlowJson && aiStore.lastUsedTools && aiStore.lastUsedTools.length > 0">
        <div class="flex items-center space-x-2 mb-3">
          <SettingsIcon class="text-red-500 w-4 h-4" />
          <h3 class="text-xs font-bold text-[var(--color-text)] uppercase">Tools Used</h3>
        </div>
        <div class="space-y-2">
          <div v-for="tool in aiStore.lastUsedTools" :key="tool" class="flex items-center justify-between p-2 rounded-lg bg-red-500/5 border border-red-500/20">
            <div class="flex items-center space-x-2">
              <SettingsIcon class="text-red-500 w-4 h-4" />
              <span class="text-xs font-medium text-[var(--color-text)]">{{ tool }}</span>
            </div>
            <span class="text-[10px] text-red-500 font-bold">NEW</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-[var(--color-surface)] rounded-xl shadow-sm border border-[var(--color-border)] overflow-hidden flex flex-col h-64">
        <div class="p-3 border-b border-[var(--color-border)] bg-[var(--color-background)] flex justify-between items-center">
          <div class="flex items-center space-x-2">
            ...
            <h3 class="text-xs font-bold text-[var(--color-text)] uppercase">JSON Preview</h3>
          </div>
          <button @click="copyJson" class="text-xs text-red-500 hover:text-red-700 font-medium" :disabled="!aiStore.lastGeneratedFlowJson">Copy</button>
        </div>
