...
> **文档用途**：供 AI 编程助手逐阶段实施该功能。每个阶段均包含完整的上下文、文件路径、代码实现和验收标准。
> **阅读方式**：按阶段顺序执行，每完成一个阶段后进行验收，再进入下一阶段。
> **项目背景**：ClearVision 是一个工业视觉检测平台，技术栈为 WPF + WebView2（桌面宿主）+ C# .NET 8（后端）+ HTML/JS（前端画布）。目标是接入 AI，使用户可以通过自然语言描述，一键生成包含算子连线的完整工程。
...
### 技术栈
- **桌面宿主**：WPF + WebView2，入口：`Acme.Product.Desktop/Program.cs`
...
public class PromptBuilder
{
    ...
    private string GetFewShotExamples() => """
        ...
        正确输出：
        {
          ...
          "operators": [
            {"tempId": "op_1", "operatorType": "ImageAcquisition", "displayName": "相机采集", "parameters": {"sourceType": "camera", "triggerMode": "Hardware"}},
            {"tempId": "op_2", "operatorType": "BarcodeRecognition", "displayName": "二维码识别", "parameters": {"CodeType": "QR", "MaxResults": "1"}},
...
### 阶段二验收标准
- [ ] `PromptBuilder` 能正确生成包含全部 46 个算子的 System Prompt
...
{
    private const double LayerWidth = 220.0;   // 每层的水平间距（像素）
    private const double NodeHeight = 160.0;   // 每个节点的垂直间距（像素）
...
> 此处应列出全部 46 个算子的类型名，实现时从 `OperatorFactory.GetAllMetadata()` 动态生成，
> 并附在 Prompt 的算子目录中。无需手动维护此列表。
...
3. **上下文长度**：当算子库继续扩展（远超 46 个算子时），System Prompt 长度会增加，可能影响 API 响应时间和成本。届时可考虑按类别分组，只传送与用户描述相关的算子子集。

4. **网络依赖**：该功能依赖网络访问 AI API，离线环境无法使用。如需离线支持，需考虑本地部署小模型（如 Ollama）替代方案。

---

*文档版本：1.0 | 生成日期：2026-02-19 | 适用项目：ClearVision*
