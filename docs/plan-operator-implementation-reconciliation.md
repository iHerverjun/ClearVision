...
- 有一批算子算法本身可用，但元数据、运行时输出、UI 可调参数不同步。
- 有一批算子实现里存在隐藏参数或未生效参数，导致调参体验和结果可解释性较差。
- 有一批算子的算法能力边界没有被产品层面显式约束，用户会把“能力边界”误解为“算法效果不好”。

## 5. 全局统计（静态扫描）

以下统计用于帮助排期，不直接等价于 bug 数量：

- 存在隐藏参数的算子：**21** 个
- 存在已声明但未读取参数的算子：**21** 个
- 存在隐藏输入的算子：**8** 个
- 存在声明输出未按同名键稳定输出的算子：**25** 个
- 额外运行时输出键漂移信号：**104** 个算子被命中

说明：最后一项包含静态扫描噪声，只能作为“输出契约普遍漂移”的信号，不能直接逐条当作 bug 清单。
...
- 已声明参数未真正生效
- 模式切换逻辑写在源码里，但没有暴露为参数
- 失败路径走“成功执行 + NG 业务结果”，但契约层没有统一约定
- 运行时附加输出很多，但端口和文档没有收敛

### 6.3 算法边界

典型表现：

- 名称看起来能力很强，但实际实现只覆盖较窄子集
- 适用场景单一，却没有在产品层面加以约束或提示
- 没有 bug，但用户预期与真实能力不匹配

## 7. 收敛原则

后续整改建议统一遵循以下原则：

1. 单一事实来源：参数、输入、输出、文档、目录索引保持一致。
...
| `GradientShapeMatch` | 契约 + 实现 | `_matcherCache` 键未包含输入模板内容；当模板来自输入端口且 `templatePath` 为空时，相同参数的不同模板会共享同一缓存键；声明 `Position` 但运行时输出 `X/Y`；固定大小显示框与真实模板尺寸无关 | 修缓存键；在端口流模板模式下避免使用仅依赖 `templatePath` 的缓存；正式输出 `Position`；区分“显示框尺寸”和“模板真实尺寸”；必要时支持关闭缓存 | 多模板切换测试 + 缓存命中测试 |
...
| `TypeConvert` | 契约 | 声明输入是 `Input`，源码还读取 `Value`；运行时输出远多于声明 `Output` | 统一输入键；决定是否保留多种附加输出；若保留则显式声明 | 输入兼容测试 + 下游转换回归测试 |
...
| 算子 | 问题类型 | 高置信度问题 | 建议动作 |
|------|------|------|------|
| `GeometricFitting` | 算法边界 + 输出契约 | 所有有效轮廓点会被合并后统一拟合；椭圆不走 RANSAC；`FitResult` 字段结构依赖 `FitType` | 增加“最大轮廓/单轮廓/全部轮廓”选择；明确椭圆鲁棒性边界；稳定结果结构 |
...
| `ArrayIndexer` | 契约 | `List` 与 `Items` 输入存在双键语义；`Item` 输出声明与运行时结果键不一致 | 统一输入输出键名 |
...
- `TemplateMatching`: `MaxMatches`
- `WidthMeasurement`: `Direction`
...
- `CameraCalibration`：单图模式应定义为“调试/验证模式”，不要作为高质量生产标定默认路径。
...
当前算子库的核心问题不是“整体算法差”，而是：

- 产品层看到的契约和实现层真实行为有明显漂移。
- 一部分用户体感上的“效果不好”，其实源自参数没生效、输出语义不清、能力边界没被说清。
- 真正需要优先修的，不是把 118 个算子全部重写，而是先把高价值算子的契约、参数和输出收敛到一致。

按本计划推进，建议先做 **Batch A + Batch B**，先把“好不好用”和“好不好解释”解决掉，再进入算法增强阶段。
