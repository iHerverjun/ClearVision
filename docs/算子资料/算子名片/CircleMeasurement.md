...
|--------|------|--------|------|------|
| `Method` | `enum` | `"HoughCircle"` | HoughCircle/霍夫圆；FitEllipse/拟合椭圆 | 算法方法选择。 |
| `MinRadius` | `int` | `10` | [0, +∞] | 最小数量或下限约束。 |
| `MaxRadius` | `int` | `200` | [0, +∞] | 最大数量或上限约束。 |
| `Dp` | `double` | `1.0` | [0.5, 4.0] | 控制“Dp”这一实现参数，建议结合现场样本调节。 |
| `MinDist` | `double` | `50.0` | [1.0, +∞] | 最小数量或下限约束。 |
| `Param1` | `double` | `100.0` | [0.0, 255.0] | 控制“Param1”这一实现参数，建议结合现场样本调节。 |
| `Param2` | `double` | `30.0` | [0.0, 255.0] | 控制“Param2”这一实现参数，建议结合现场样本调节。 |
...
| `Radius` | 半径 | `Float` | 输出本算子的处理结果。 |
| `Center` | 圆心 | `Point` | 输出点位结果。 |
| `Circle` | 圆数据 | `CircleData` | 输出本算子的处理结果。 |
| `CircleCount` | 圆数量 | `Integer` | 输出本算子的处理结果。 |
...
| `Radius` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Circularity` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Circles` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `CircleCount` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `CircleDataList` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
...
3. 声明输出 `Center` 与当前运行时附加字段不完全一致，集成时应以实际输出字典为准。
