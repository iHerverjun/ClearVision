...
|--------|------|--------|------|------|
| `Element1Type` | `enum` | `"Auto"` | Auto/Auto；Point/Point；Line/Line；Circle/Circle | 操作类型或转换类型。 |
| `Element2Type` | `enum` | `"Auto"` | Auto/Auto；Point/Point；Line/Line；Circle/Circle | 操作类型或转换类型。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Element1` | Element 1 | `Any` | Yes | 提供算法执行所需输入。 |
| `Element2` | Element 2 | `Any` | Yes | 提供算法执行所需输入。 |
...
| `Intersection1` | Intersection 1 | `Point` | 输出点位结果。 |
| `Intersection2` | Intersection 2 | `Point` | 输出点位结果。 |
| `MeasureType` | Measure Type | `String` | 输出本算子的处理结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
