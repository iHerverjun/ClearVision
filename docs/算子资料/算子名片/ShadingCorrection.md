...
|--------|------|--------|------|------|
| `Method` | `enum` | `"GaussianModel"` | DivideByBackground/DivideByBackground；GaussianModel/GaussianModel；MorphologicalTopHat/MorphologicalTopHat | 算法方法选择。 |
| `KernelSize` | `int` | `51` | [3, 501] | 局部窗口或卷积核尺寸。 |
...
| `Background` | Background | `Image` | No | 输入待处理图像。 |
...
| `shading` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `DivideByBackground` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
