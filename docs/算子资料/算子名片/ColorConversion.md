...
|--------|------|--------|------|------|
| `ConversionCode` | `enum` | `"BGR2GRAY"` | BGR2GRAY/BGR转灰度；BGR2HSV/BGR转HSV；BGR2Lab/BGR转Lab；BGR2YUV/BGR转YUV；GRAY2BGR/灰度转BGR；HSV2BGR/HSV转BGR | 该参数用于在多个实现分支之间切换。 |
...
| `ConversionCode` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `BGR2GRAY` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
