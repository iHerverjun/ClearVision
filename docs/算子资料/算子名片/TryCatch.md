...
| `CatchOutputError` | `bool` | `true` | - | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
| `CatchOutputStackTrace` | `bool` | `false` | - | 该参数用于控制当前实现行为，建议结合现场样本调节。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Input` | 输入 | `Any` | No | 提供流程数据输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Try` | Try分支 | `Any` | 输出流程数据结果。 |
| `Catch` | Catch分支 | `Any` | 输出流程数据结果。 |
| `Error` | 错误信息 | `String` | 输出流程数据结果。 |
| `HasError` | 是否有错 | `Boolean` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
...
1. 声明输出 `Try` 与当前运行时附加字段不完全一致，集成时应以实际输出字典为准。
