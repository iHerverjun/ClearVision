...
|--------|------|--------|------|------|
| `Top` | `int` | `0` | [0, 10000] | 控制“Top”这一实现参数，建议结合现场样本调节。 |
| `Bottom` | `int` | `0` | [0, 10000] | 控制“Bottom”这一实现参数，建议结合现场样本调节。 |
| `Left` | `int` | `0` | [0, 10000] | 控制“Left”这一实现参数，建议结合现场样本调节。 |
| `Right` | `int` | `0` | [0, 10000] | 控制“Right”这一实现参数，建议结合现场样本调节。 |
| `BorderType` | `enum` | `"Constant"` | Constant/Constant；Replicate/Replicate；Reflect/Reflect；Wrap/Wrap | 操作类型或转换类型。 |
| `Color` | `string` | `"#000000"` | - | 控制“Color”这一实现参数，建议结合现场样本调节。 |
...
| `border` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Constant` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
