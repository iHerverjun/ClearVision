...
|--------|------|--------|------|------|
| `VariableName` | `string` | `"counter"` | - | 计数器变量名称 |
| `Delta` | `int` | `1` | - | 每次递增的值（可为负数实现递减） |
| `ResetCondition` | `enum` | `"None"` | None/不重置；GreaterThan/大于阈值；LessThan/小于阈值；Equal/等于阈值 | 满足条件时重置计数器 |
| `ResetThreshold` | `int` | `100` | - | 用于判定、分割或筛选的阈值。 |
| `ResetValue` | `int` | `0` | - | 重置后的起始值 |
...
|------|------|------|------|
| `VariableName` | 变量名 | `String` | 输出流程数据结果。 |
| `PreviousValue` | 前值 | `Integer` | 输出流程数据结果。 |
| `NewValue` | 新值 | `Integer` | 输出流程数据结果。 |
| `Delta` | 增量 | `Integer` | 输出流程数据结果。 |
| `WasReset` | 是否已重置 | `Boolean` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
