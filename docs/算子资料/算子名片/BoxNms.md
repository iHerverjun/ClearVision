...
|--------|------|--------|------|------|
| `IouThreshold` | `double` | `0.45` | [0.1, 1.0] | 用于判定、分割或筛选的阈值。 |
| `ScoreThreshold` | `double` | `0.25` | [0.0, 1.0] | 用于判定、分割或筛选的阈值。 |
| `MaxDetections` | `int` | `100` | [1, 1000] | 最大数量或上限约束。 |
...
| `nms` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
