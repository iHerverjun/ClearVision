# Morphological Operation / MorphologicalOperation
...
|--------|------|--------|------|------|
| `Operation` | `enum` | `"Close"` | Erode/Erode；Dilate/Dilate；Open/Open；Close/Close；Gradient/Gradient；TopHat/TopHat | 该参数用于在多个实现分支之间切换。 |
| `KernelShape` | `enum` | `"Rect"` | Rect/Rect；Cross/Cross；Ellipse/Ellipse | 该参数用于在多个实现分支之间切换。 |
| `KernelWidth` | `int` | `3` | [1, 51] | 控制“KernelWidth”这一实现参数，建议结合现场样本调节。 |
| `KernelHeight` | `int` | `3` | [1, 51] | 控制“KernelHeight”这一实现参数，建议结合现场样本调节。 |
| `Iterations` | `int` | `1` | [1, 10] | 控制“Iterations”这一实现参数，建议结合现场样本调节。 |
| `AnchorX` | `int` | `-1` | - | 控制“AnchorX”这一实现参数，建议结合现场样本调节。 |
| `AnchorY` | `int` | `-1` | - | 控制“AnchorY”这一实现参数，建议结合现场样本调节。 |
...
| `KernelShape` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `KernelSize` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Iterations` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
