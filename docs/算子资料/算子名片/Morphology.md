...
|--------|------|--------|------|------|
| `Operation` | `string` | `"Erode"` | - | 控制“Operation”这一实现参数，建议结合现场样本调节。 |
| `KernelSize` | `int` | `3` | [1, 51] | 局部窗口或卷积核尺寸。 |
| `KernelShape` | `string` | `"Rect"` | - | 控制“KernelShape”这一实现参数，建议结合现场样本调节。 |
...
| `Morphology` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Legacy` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
...
| `LegacyCompatible` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
