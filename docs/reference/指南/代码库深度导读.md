...
- `算子资料/算子目录.md` 当前同步统计：算子总数 **118**，目录平均质量分 **88.6**.
...
- WebMessage / SharedBuffer：用于 WebView2 深度集成、AI 生成、文件选择、二维平面标定、共享图像缓冲区等。
...
- 前端：`calibrationWizard.js`、`planarScaleOffsetCalibWizard.js`
- 后端消息：`planar2d:solve`、`planar2d:save`
- 服务：`PlanarScaleOffsetCalibrationService.cs`
...
| `HandlePlanarScaleOffsetSolveCommand(string messageJson)`        | 触发二维平面比例偏移标定求解              |
| `HandlePlanarScaleOffsetSaveCommand(string messageJson)`         | 保存二维平面比例偏移标定结果              |
...
#### `PlanarScaleOffsetCalibrationService.cs`

| 方法                                                                       | 作用                 |
| -------------------------------------------------------------------------- | -------------------- |
| `SolveAsync(List<PlanarScaleOffsetCalibrationPoint> points)`             | 根据点对求解二维比例/偏移标定结果 |
| `SaveCalibrationAsync(PlanarScaleOffsetCalibrationResult result, string fileName)` | 保存二维平面标定文件 |
...
| 方法                            | 作用               |
| ------------------------------- | ------------------ |
| `constructor(cameraManager)`  | 初始化二维平面比例偏移标定向导 |
| `createUI()`                  | 创建 DOM           |
| `attachEvents()`              | 绑定事件           |
...
- `docs/reference/指南/用户指南.md`
- `docs/reference/指南/部署指南.md`
- `算子资料/算子目录.md`
- `算子资料/算子名片/*.md`
- `docs/reference/审计/当前Bug与架构隐患复核-2026-03-12.md`
- `docs/reference/历史资料/开发规划综合文档.md`
