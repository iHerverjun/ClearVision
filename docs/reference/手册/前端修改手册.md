...
> Theme System Update (2026-04)
>
> - Main WebView now uses explicit `html[data-theme="dark"]` and `html[data-theme="light"]` baselines
> - `:root` is reserved for non-theme structural variables only
> - Runtime theme state is managed through `document.documentElement.dataset.theme`
> - `general.theme` is the persisted source of truth; `localStorage.cv_theme` is only a startup cache
> - This round does not cover `login.html`
## 1. 项目架构总览
