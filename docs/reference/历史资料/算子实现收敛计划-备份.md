...
- `ArrayIndexer`: `LabelFilter` 鉁?宸叉毚闇?
...
- `TemplateMatching`: `MaxMatches` 鉁?宸插疄鐜?
- `WidthMeasurement`: `Direction` 鉁?宸插疄鐜?
...
- `ArrayIndexer`: `Items` 鉁?宸叉敹鏁涳紙浼樺厛 `List`锛屽吋瀹?`Items`锛?
...
- `TypeConvert`: `Value` 鉁?宸叉敹鏁涳紙绉婚櫎 `Value`锛岀粺涓€涓?`Input`锛?
...
褰撳墠绠楀瓙搴撶殑鏍稿績闂涓嶆槸"鏁翠綋绠楁硶宸?锛岃€屾槸锛?

- 浜у搧灞傜湅鍒扮殑濂戠害鍜屽疄鐜板眰鐪熷疄琛屼负鏈夋槑鏄炬紓绉汇€?
- 涓€閮ㄥ垎鐢ㄦ埛浣撴劅涓婄殑"鏁堟灉涓嶅ソ"锛屽叾瀹炴簮鑷弬鏁版病鐢熸晥銆佽緭鍑鸿涔変笉娓呫€佽兘鍔涜竟鐣屾病琚娓呫€?
- 鐪熸闇€瑕佷紭鍏堜慨鐨勶紝涓嶆槸鎶?118 涓畻瀛愬叏閮ㄩ噸鍐欙紝鑰屾槸鍏堟妸楂樹环鍊肩畻瀛愮殑濂戠害銆佸弬鏁板拰杈撳嚭鏀舵暃鍒颁竴鑷淬€?

鎸夋湰璁″垝鎺ㄨ繘锛屽缓璁厛鍋?**Batch A + Batch B**锛屽厛鎶?濂戒笉濂界敤"鍜?濂戒笉濂借В閲?瑙ｅ喅鎺夛紝鍐嶈繘鍏ョ畻娉曞寮洪樁娈点€?
