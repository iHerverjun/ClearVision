...
|--------|------|--------|------|------|
| `Channel` | `enum` | `"Gray"` | Gray/Gray；R/R；G/G；B/B | 该参数用于在多个实现分支之间切换。 |
| `BinCount` | `int` | `256` | [2, 1024] | 控制“BinCount”这一实现参数，建议结合现场样本调节。 |
| `RoiX` | `int` | `0` | [0, +∞] | 限定处理区域。 |
| `RoiY` | `int` | `0` | [0, +∞] | 限定处理区域。 |
| `RoiW` | `int` | `0` | [0, +∞] | 限定处理区域。 |
| `RoiH` | `int` | `0` | [0, +∞] | 限定处理区域。 |
...
| `Mean` | Mean | `Float` | 输出本算子的处理结果。 |
| `StdDev` | StdDev | `Float` | 输出本算子的处理结果。 |
| `Mode` | Mode | `Integer` | 输出本算子的处理结果。 |
| `Median` | Median | `Integer` | 输出本算子的处理结果。 |
| `Peak` | Peak | `Integer` | 输出本算子的处理结果。 |
| `Valley` | Valley | `Integer` | 输出本算子的处理结果。 |
### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|
| `Width` | `Integer` | 输出图像宽度。 |
| `Height` | `Integer` | 输出图像高度。 |
| `histogram` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Mean` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `StdDev` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Mode` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Median` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Peak` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Valley` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Gray` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
