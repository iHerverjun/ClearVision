...
|--------|------|--------|------|------|
| `Protocol` | `enum` | `"TCP"` | TCP/TCP；RTU/RTU | 该参数用于在多个实现分支之间切换。 |
| `IpAddress` | `string` | `"192.168.1.1"` | - | 控制“IpAddress”这一实现参数，建议结合现场样本调节。 |
| `Port` | `int` | `502` | [1, 65535] | 通信端口。 |
| `SlaveId` | `int` | `1` | [1, 247] | 控制“SlaveId”这一实现参数，建议结合现场样本调节。 |
| `RegisterAddress` | `int` | `0` | - | 控制“RegisterAddress”这一实现参数，建议结合现场样本调节。 |
| `RegisterCount` | `int` | `1` | [1, 125] | 控制“RegisterCount”这一实现参数，建议结合现场样本调节。 |
| `FunctionCode` | `enum` | `"ReadHolding"` | ReadCoils/读线圈；ReadHolding/读保持寄存器；WriteSingle/写单寄存器；WriteMultiple/写多寄存器 | 该参数用于在多个实现分支之间切换。 |
