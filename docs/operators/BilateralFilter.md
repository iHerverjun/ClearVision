...
该算子使用双边滤波同时考虑空间距离与像素差异，在保边前提下降低噪声。
...
|--------|------|--------|------|------|
| `Diameter` | `int` | `9` | [1, 25] | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
| `SigmaColor` | `double` | `75.0` | [1.0, 255.0] | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
| `SigmaSpace` | `double` | `75.0` | [1.0, 255.0] | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
