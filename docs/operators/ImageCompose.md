...
|--------|------|--------|------|------|
| `Mode` | `enum` | `"Horizontal"` | Horizontal/Horizontal；Vertical/Vertical；Grid/Grid；ChannelMerge/ChannelMerge | 工作模式选择。 |
| `Padding` | `int` | `0` | [0, 1000] | 控制“Padding”这一实现参数，建议结合现场样本调节。 |
| `BackgroundColor` | `string` | `"#000000"` | - | 控制“BackgroundColor”这一实现参数，建议结合现场样本调节。 |
...
| `Image3` | Image 3 | `Image` | No | 输入待处理图像。 |
| `Image4` | Image 4 | `Image` | No | 输入待处理图像。 |
...
| `compose` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
