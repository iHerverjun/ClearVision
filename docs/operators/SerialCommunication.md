...
| `BaudRate` | `enum` | `"9600"` | 9600/9600；19200/19200；38400/38400；57600/57600；115200/115200 | 该参数用于在多个实现分支之间切换。 |
| `DataBits` | `int` | `8` | [5, 8] | 控制“DataBits”这一实现参数，建议结合现场样本调节。 |
| `StopBits` | `enum` | `"One"` | One/1；OnePointFive/1.5；Two/2 | 该参数用于在多个实现分支之间切换。 |
| `Parity` | `enum` | `"None"` | None/无；Odd/奇校验；Even/偶校验 | 该参数用于在多个实现分支之间切换。 |
| `SendData` | `string` | `""` | - | 控制“SendData”这一实现参数，建议结合现场样本调节。 |
| `Encoding` | `enum` | `"UTF8"` | UTF8/UTF-8；ASCII/ASCII；HEX/HEX | 文本编码。 |
| `TimeoutMs` | `int` | `3000` | [100, 30000] | 超时时间。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Data` | 发送数据 | `Any` | No | 提供通信请求所需输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Response` | 接收数据 | `Any` | 输出通信状态或响应结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
