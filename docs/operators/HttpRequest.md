...
|--------|------|--------|------|------|
| `Url` | `string` | `"http://localhost:5000/api"` | - | 控制“Url”这一实现参数，建议结合现场样本调节。 |
| `Method` | `enum` | `"POST"` | GET/GET；POST/POST；PUT/PUT；DELETE/DELETE | 算法方法选择。 |
| `Timeout` | `int` | `5000` | - | 超时时间。 |
| `MaxRetries` | `int` | `3` | - | 最大数量或上限约束。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Body` | 请求体 | `String` | No | 提供通信请求所需输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Response` | 响应内容 | `String` | 输出通信状态或响应结果。 |
| `StatusCode` | 状态码 | `Integer` | 输出通信状态或响应结果。 |
