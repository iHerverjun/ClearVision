...
|--------|------|--------|------|------|
| `Direction` | `enum` | `"Auto"` | Horizontal/Horizontal；Vertical/Vertical；Auto/Auto | 该参数用于在多个实现分支之间切换。 |
| `MinGap` | `double` | `0.0` | [0.0, 1000000.0] | 最小数量或下限约束。 |
| `MaxGap` | `double` | `0.0` | [0.0, 1000000.0] | 最大数量或上限约束。 |
| `ExpectedCount` | `int` | `0` | [0, 10000] | 控制“ExpectedCount”这一实现参数，建议结合现场样本调节。 |
...
| `Points` | Points | `PointList` | No | 提供点位输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Image` | Image | `Image` | 输出处理后的结果图像。 |
| `Gaps` | Gaps | `Any` | 输出本算子的处理结果。 |
| `MeanGap` | Mean Gap | `Float` | 输出本算子的处理结果。 |
| `MinGap` | Min Gap | `Float` | 输出本算子的处理结果。 |
| `MaxGap` | Max Gap | `Float` | 输出本算子的处理结果。 |
...
| `gap` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Gaps` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `MeanGap` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `MinGap` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `MaxGap` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Count` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
