...
|--------|------|--------|------|------|
| `KernelSize` | `int` | `3` | [1, 7] | 必须是奇数 |
| `Scale` | `double` | `1.0` | [0.1, 10.0] | 缩放或比例参数。 |
| `SharpenStrength` | `double` | `1.0` | [0, 5.0] | 控制“SharpenStrength”这一实现参数，建议结合现场样本调节。 |
...
| `Scale` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `SharpenStrength` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
