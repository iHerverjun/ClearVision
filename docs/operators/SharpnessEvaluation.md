...
|--------|------|--------|------|------|
| `Method` | `enum` | `"Laplacian"` | Laplacian/Laplacian；Brenner/Brenner；Tenengrad/Tenengrad；SMD/SMD | 算法方法选择。 |
| `Threshold` | `double` | `100.0` | [0.0, +∞] | 用于判定、分割或筛选的阈值。 |
| `RoiX` | `int` | `0` | - | 限定处理区域。 |
| `RoiY` | `int` | `0` | - | 限定处理区域。 |
| `RoiW` | `int` | `0` | - | 限定处理区域。 |
| `RoiH` | `int` | `0` | - | 限定处理区域。 |
...
|------|------|------|------|
| `Score` | Score | `Float` | 输出本算子的处理结果。 |
| `IsSharp` | Is Sharp | `Boolean` | 输出本算子的处理结果。 |
...
| `sharpness` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Score` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `IsSharp` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Method` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Laplacian` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
