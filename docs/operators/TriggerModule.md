...
|--------|------|--------|------|------|
| `TriggerMode` | `enum` | `"Software"` | Software/Software；Timer/Timer；ExternalSignal/ExternalSignal | 工作模式选择。 |
| `Interval` | `int` | `1000` | [1, 3600000] | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
| `AutoRepeat` | `bool` | `true` | - | 该参数用于控制当前实现行为，建议结合现场样本调节。 |
...
|------|------|------|------|
| `Triggered` | Triggered | `Boolean` | 输出流程数据结果。 |
| `Timestamp` | Timestamp | `String` | 输出流程数据结果。 |
| `TriggerCount` | Trigger Count | `Integer` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
