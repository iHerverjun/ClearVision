...
|--------|------|--------|------|------|
| `IpAddress` | `string` | `"192.168.0.1"` | - | 控制“IpAddress”这一实现参数，建议结合现场样本调节。 |
| `Port` | `int` | `102` | [1, 65535] | 通信端口。 |
| `CpuType` | `enum` | `"S71200"` | S7200/S7-200；S7200Smart/S7-200 Smart；S7300/S7-300；S7400/S7-400；S71200/S7-1200；S71500/S7-1500 | 操作类型或转换类型。 |
| `Rack` | `int` | `0` | [0, 15] | 控制“Rack”这一实现参数，建议结合现场样本调节。 |
| `Slot` | `int` | `1` | [0, 15] | 控制“Slot”这一实现参数，建议结合现场样本调节。 |
| `Address` | `string` | `"DB1.DBW100"` | - | 控制“Address”这一实现参数，建议结合现场样本调节。 |
| `DataType` | `enum` | `"Word"` | Bit/位 (Bool)；Byte/字节 (Byte)；Word/字 (Word/UInt16)；Int16/短整型 (Int16)；DWord/双字 (DWord/UInt32)；Int32/整型 (Int32) | 操作类型或转换类型。 |
