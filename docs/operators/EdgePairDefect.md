...
|--------|------|--------|------|------|
| `ExpectedWidth` | `double` | `20.0` | [0.0, 100000.0] | 控制“ExpectedWidth”这一实现参数，建议结合现场样本调节。 |
| `Tolerance` | `double` | `2.0` | [0.0, 100000.0] | 控制“Tolerance”这一实现参数，建议结合现场样本调节。 |
| `NumSamples` | `int` | `100` | [5, 5000] | 控制“NumSamples”这一实现参数，建议结合现场样本调节。 |
| `EdgeMethod` | `enum` | `"Canny"` | Canny/Canny；Sobel/Sobel | 算法方法选择。 |
...
| `Line1` | Line 1 | `LineData` | No | 提供线几何输入。 |
| `Line2` | Line 2 | `LineData` | No | 提供线几何输入。 |
...
| `MaxDeviation` | Max Deviation | `Float` | 输出本算子的处理结果。 |
| `Deviations` | Deviations | `Any` | 输出本算子的处理结果。 |
...
| `MaxDeviation` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Deviations` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Canny` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
