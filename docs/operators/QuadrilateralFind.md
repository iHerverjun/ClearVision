...
| `ConvexOnly` | `bool` | `false` | - | 控制“ConvexOnly”这一实现参数，建议结合现场样本调节。 |
...
| `Vertices` | Vertices | `PointList` | 输出本算子的处理结果。 |
| `Count` | Count | `Integer` | 输出本算子的处理结果。 |
| `Area` | Area | `Float` | 输出本算子的处理结果。 |
...
| `quadrilateral` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Vertices` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Count` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Area` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Center` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
