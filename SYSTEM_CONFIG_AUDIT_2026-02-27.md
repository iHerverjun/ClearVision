...
- 约束: 本次仅审计，不改代码
- 最后更新: 2026-02-27 19:00 — 第一梯队修复核查完成，追加下一步规划
...
1. "PLC 通讯配置"以占位 UI 为主。除协议/IP/端口可随"保存所有更改"写入配置外，连接测试、地址映射、同步配置等均未形成真实闭环。
2. 真实有效程度最高的模块: `AI 大模型管理`、`相机管理`、`用户管理（管理员）`。
3. 大量设置项处于"可保存但不生效"的半完成状态，尤其在常规设置、文件存储、执行与运行。
4. 第一梯队修复（4 项高风险硬编码/覆写问题）已于 2026-02-27 完成并通过质量核查。
...
| 模块 | 判定 | 结论 |
|---|---|---|
| 全局保存（保存所有更改） | 半有效 | 保存链路存在；`activeCameraId` 覆写问题 ✅ 已修复 |
| 常规设置 | 半有效/占位混合 | 主题可本地生效；"保存常规设置"按钮无绑定 |
| 通讯连接（PLC） | 占位为主 | 核心动作未绑定；`heartbeatIntervalMs` 硬编码 ✅ 已修复 |
| 文件存储 | 半有效/占位混合 | 路径和策略可保存；阈值硬编码 ✅ 已修复；无运行时消费 |
| 执行与运行 | 半有效/占位混合 | `AutoRun/连续NG阈值`可保存但无后续消费；其余占位 |
| 相机管理 | 真实有效（部分占位）| 搜索/绑定/删除/参数/软触发闭环完整；帧率宽高为占位 |
| AI 大模型 | 真实有效（性能卡占位）| CRUD/激活/测试/持久化完整；右侧性能卡为假数据 |
| 用户管理 | 真实有效（策略区占位）| CRUD/启停/重置密码完整；死代码 ✅ 已清理；安全策略区未接后端 |
...
- 残留风险:
  - 多步骤保存（settings → bindings → AI form）仍为非事务式，存在部分成功窗口。
  - `_saveCurrentForm()` 隐式保存当前 AI 表单，用户在非 AI tab 时可能无感知。
...
- 判定: 占位伪代码
- 按钮无 `id` 与监听 (`settingsView.js:936-939`)。
...
- 判定: 半有效
- 字段可保存入配置，但未发现运行时从 `AppConfig.Communication` 读取并驱动 PLC 通讯。
- `heartbeatIntervalMs`: ✅ 已修复，从 config 透传（`settingsView.js:1765-1768`）。

#### 4.3.2 连接测试/地址映射/同步按钮

- 判定: 占位伪代码
- 全部按钮无事件绑定；地址映射为硬编码 5 行示例；"已连接"为静态文案。
...
- 判定: 半有效
- `ImageSavePath/SavePolicy` 可保存，但无运行时消费。
...
1. PLC 配置页面"看起来可用、实际无效"，易导致现场误操作判断。
2. 全局保存分步提交（settings → bindings → AI），可能产生部分成功状态。
3. `_saveCurrentForm()` 隐式保存 AI 表单，用户不在 AI tab 时无感知。
4. 多数配置"可保存不可生效"，形成配置管理错觉。

### 6.3 🟢 低风险

1. "保存常规设置"按钮存在但无功能，可能造成用户疑惑。
2. 相机连接状态为推断值而非实时探测。
...
> 以下为当前最有价值的改进方向，按优先级排序。每项标注了预估工作量和涉及文件。
