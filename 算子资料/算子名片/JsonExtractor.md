...
|--------|------|--------|------|------|
| `JsonPath` | `string` | `"$.data"` | - | 文件或资源路径。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Json` | JSON字符串 | `String` | Yes | 提供流程数据输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Value` | 提取值 | `Any` | 输出流程数据结果。 |
| `IsSuccess` | 是否成功 | `Boolean` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
...
1. 参数 `JsonPath` 已在元数据中声明，但从源码看当前没有明显被执行逻辑实际使用。
