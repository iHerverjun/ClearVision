...
|--------|------|--------|------|------|
| `KernelSize` | `int` | `5` | [1, 31] | 局部窗口或卷积核尺寸。 |
| `SigmaX` | `double` | `1.0` | [0.1, 10.0] | 最大数量或上限约束。 |
| `SigmaY` | `double` | `0.0` | [0.0, 10.0] | 控制“SigmaY”这一实现参数，建议结合现场样本调节。 |
| `BorderType` | `enum` | `"4"` | 0/Constant；1/Replicate；2/Reflect；3/Wrap；4/Default | 操作类型或转换类型。 |
...
| `gaussian` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
