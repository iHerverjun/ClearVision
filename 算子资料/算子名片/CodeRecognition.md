...
|--------|------|--------|------|------|
| `CodeType` | `enum` | `"All"` | All/全部；QR/QR码；Code128/Code128；DataMatrix/DataMatrix；EAN13/EAN-13；Code39/Code39 | 操作类型或转换类型。 |
| `MaxResults` | `int` | `10` | [1, 100] | 最大数量或上限约束。 |
...
| `Text` | 识别内容 | `String` | 输出本算子的处理结果。 |
| `CodeCount` | 识别数量 | `Integer` | 输出本算子的处理结果。 |
| `CodeType` | 条码类型 | `String` | 输出本算子的处理结果。 |
...
| `Text` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Format` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Points` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `CodeType` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `CodeResults` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `ResultCount` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Codes` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
