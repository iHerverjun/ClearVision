...
|--------|------|--------|------|------|
| `Method` | `enum` | `"MinMax"` | MinMax/MinMax；ZScore/ZScore；Histogram/Histogram | 算法方法选择。 |
| `Alpha` | `double` | `0.0` | - | 控制“Alpha”这一实现参数，建议结合现场样本调节。 |
| `Beta` | `double` | `255.0` | - | 控制“Beta”这一实现参数，建议结合现场样本调节。 |
...
| `normalize` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `MinMax` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
