...
|--------|------|--------|------|------|
| `Mode` | `enum` | `"External"` | External/外部；List/列表；Tree/树 | 工作模式选择。 |
| `Method` | `enum` | `"Simple"` | Simple/简单；None/无 | 算法方法选择。 |
| `MinArea` | `int` | `100` | - | 最小数量或下限约束。 |
| `MaxArea` | `int` | `100000` | - | 最大数量或上限约束。 |
| `Threshold` | `double` | `127.0` | - | 用于判定、分割或筛选的阈值。 |
...
| `Contours` | 轮廓数据 | `Contour` | 输出本算子的处理结果。 |
...
| `Id` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
