...
| `LabelsPath` | `file` | "" | - | 自定义标签文件路径（每行一个标签），为空则使用COCO 80类或自动查找模型目录下的labels.txt |
| `EnableInternalNms` | `bool` | true | - | 关闭后输出置信度筛选后的候选框，由下游 BoxNms 负责唯一 NMS。 |
...
|------|------|------|------|
| `Image` | 结果图像 | `Image` | - |
| `OriginalImage` | 原始图像 | `Image` | - |
| `DetectionList` | 检测列表 | `DetectionList` | - |
| `Defects` | 缺陷列表 | `DetectionList` | - |
| `DefectCount` | 缺陷数量 | `Integer` | - |
| `Objects` | 目标列表 | `DetectionList` | - |
| `ObjectCount` | 目标数量 | `Integer` | - |
