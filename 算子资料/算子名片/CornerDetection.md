...
|--------|------|--------|------|------|
| `Method` | `enum` | `"ShiTomasi"` | Harris/Harris；ShiTomasi/ShiTomasi | 算法方法选择。 |
| `MaxCorners` | `int` | `100` | [1, 5000] | 最大数量或上限约束。 |
| `QualityLevel` | `double` | `0.01` | [1E-06, 1.0] | 控制“QualityLevel”这一实现参数，建议结合现场样本调节。 |
| `MinDistance` | `double` | `10.0` | [0.0, 10000.0] | 最小数量或下限约束。 |
| `BlockSize` | `int` | `3` | [2, 31] | 控制“BlockSize”这一实现参数，建议结合现场样本调节。 |
...
| `Corners` | Corners | `PointList` | 输出本算子的处理结果。 |
...
| `corner` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Corners` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Count` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Harris` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
