...
|--------|------|--------|------|------|
| `LabelBy` | `enum` | `"Area"` | Area/Area；Circularity/Circularity；AspectRatio/AspectRatio；Position/Position | 该参数用于在多个实现分支之间切换。 |
| `Thresholds` | `string` | `"[` | - | 用于判定、分割或筛选的阈值。 |
| `DrawLabels` | `bool` | `true` | - | 控制“DrawLabels”这一实现参数，建议结合现场样本调节。 |
...
| `Blobs` | Blobs | `Contour` | No | 提供算法执行所需输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Image` | Image | `Image` | 输出处理后的结果图像。 |
| `Labels` | Labels | `Any` | 输出本算子的处理结果。 |
...
| `blob` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Index` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Label` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Area` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Circularity` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `AspectRatio` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
