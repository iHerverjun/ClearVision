...
|--------|------|--------|------|------|
| `Condition` | `enum` | `"GreaterThan"` | GreaterThan/大于；GreaterThanOrEqual/大于等于；LessThan/小于；LessThanOrEqual/小于等于；Equal/等于；NotEqual/不等于 | 该参数用于在多个实现分支之间切换。 |
| `CompareValue` | `double` | `0.0` | - | 当 ValueB 未连线时使用此值 |
| `Tolerance` | `double` | `0.0001` | [0.0, +∞] | 等于/不等于判断的容差 |
| `RangeMin` | `double` | `0.0` | - | InRange 模式的下限 |
| `RangeMax` | `double` | `1.0` | - | InRange 模式的上限 |
...
|------|------|------|------|
| `Result` | 判定结果 | `Boolean` | 输出流程数据结果。 |
| `Difference` | 差值 | `Float` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
...
1. 声明输出 `Result` 与当前运行时附加字段不完全一致，集成时应以实际输出字典为准。
