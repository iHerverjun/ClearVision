...
|--------|------|--------|------|------|
| `Method` | `enum` | `"Global"` | Global/全局均衡化；CLAHE/CLAHE自适应 | 算法方法选择。 |
| `ClipLimit` | `double` | `2.0` | [0.0, 100.0] | 控制“ClipLimit”这一实现参数，建议结合现场样本调节。 |
| `TileGridSize` | `int` | `8` | [1, 64] | 控制“TileGridSize”这一实现参数，建议结合现场样本调节。 |
...
| `global` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
...
3. 参数 `TileGridSize` 已在元数据中声明，但从源码看当前没有明显被执行逻辑实际使用。
