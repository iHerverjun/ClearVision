...
|--------|------|--------|------|------|
| `Rows` | `int` | `2` | [1, 100] | 控制“Rows”这一实现参数，建议结合现场样本调节。 |
| `Cols` | `int` | `2` | [1, 100] | 控制“Cols”这一实现参数，建议结合现场样本调节。 |
| `Overlap` | `int` | `0` | [0, 10000] | 控制“Overlap”这一实现参数，建议结合现场样本调节。 |
| `OutputMode` | `enum` | `"Array"` | Array/Array；Sequential/Sequential | 工作模式选择。 |
...
| `tile` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `Tiles` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
...
2. 参数 `OutputMode` 已在元数据中声明，但从源码看当前没有明显被执行逻辑实际使用。
