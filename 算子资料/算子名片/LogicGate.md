...
|--------|------|--------|------|------|
| `Operation` | `enum` | `"AND"` | AND/AND (与)；OR/OR (或)；NOT/NOT (非)；XOR/XOR (异或)；NAND/NAND (与非)；NOR/NOR (或非) | 该参数用于在多个实现分支之间切换。 |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `InputA` | 输入 A | `Boolean` | Yes | 提供流程数据输入。 |
| `InputB` | 输入 B | `Boolean` | No | 提供流程数据输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Result` | 输出 | `Boolean` | 输出流程数据结果。 |
## 性能特征 / Performance
| 指标 (Metric) | 值 (Value) |
|------|------|
