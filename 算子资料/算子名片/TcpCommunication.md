...
|--------|------|--------|------|------|
| `Mode` | `enum` | `"Client"` | Client/客户端；Server/服务器 | 工作模式选择。 |
| `IpAddress` | `string` | `"127.0.0.1"` | - | 控制“IpAddress”这一实现参数，建议结合现场样本调节。 |
| `Port` | `int` | `8080` | [1, 65535] | 通信端口。 |
| `SendData` | `string` | `""` | - | 控制“SendData”这一实现参数，建议结合现场样本调节。 |
| `Timeout` | `int` | `5000` | [100, 30000] | 超时时间。 |
| `Encoding` | `enum` | `"UTF8"` | UTF8/UTF-8；ASCII/ASCII；GBK/GBK | 文本编码。 |
