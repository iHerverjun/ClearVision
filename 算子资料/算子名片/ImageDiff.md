...
|------|------|------|------|------|
| `BaseImage` | 基准图 | `Image` | Yes | 输入待处理图像。 |
| `CompareImage` | 对比图 | `Image` | Yes | 输入待处理图像。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `DiffImage` | 差异图 | `Image` | 输出处理后的结果图像。 |
| `DiffRate` | 差异率 | `Float` | 输出本算子的处理结果。 |
...
2. 声明输出 `DiffImage` 与当前运行时附加字段不完全一致，集成时应以实际输出字典为准。
