...
|--------|------|--------|------|------|
| `ClipLimit` | `double` | `2.0` | [0, 40] | 对比度限制阈值，防止过度放大噪声 |
| `TileWidth` | `int` | `8` | [2, 64] | 控制“TileWidth”这一实现参数，建议结合现场样本调节。 |
| `TileHeight` | `int` | `8` | [2, 64] | 控制“TileHeight”这一实现参数，建议结合现场样本调节。 |
| `ColorSpace` | `enum` | `"Lab"` | Lab/Lab - L通道；HSV/HSV - V通道；Gray/灰度；All/所有通道 | 该参数用于在多个实现分支之间切换。 |
...
|------|------|------|------|
| `Image` | 增强图像 | `Image` | 输出处理后的结果图像。 |
### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|
| `Width` | `Integer` | 输出图像宽度。 |
| `Height` | `Integer` | 输出图像高度。 |
| `ClipLimit` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `TileSize` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `ColorSpace` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
| `lab` | `Auto` | 当前实现中的运行时附加字段，具体语义以源码输出逻辑为准。 |
